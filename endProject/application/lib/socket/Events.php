...
class Events
{
    ...
    public static function onMessage($client_id, $data)
    {
        ...
        // 非法参数
        if (!is_array($data) || !array_key_exists('type', $data) || !array_key_exists('token', $data) || $data['type'] !== 'bind' || empty($data['token'])) {
            return;
        }
        $user = Cache::get($data['token']);
        if (!$user) {
            return Gateway::sendToCurrentClient(json_encode(['type'=>'bind','msg'=>'非法token，禁止操作','status'=>false]));
        }
        // 获取用户id
        $userId = array_key_exists('type', $user) ? $user['user_id'] : $user['id'];
        // 验证第三方是否绑定手机
        if ($userId < 1) {
            return Gateway::sendToCurrentClient(json_encode(['type'=>'bind','msg'=>'请先绑定手机','status'=>false]));
        }
        ...
        // 验证用户状态
        if ($User->status == 0) {
            return Gateway::sendToCurrentClient(json_encode(['type'=>'bind','msg'=>'当前用户被禁用','status'=>false]));
        }
        // 绑定
